    ...
    def forward(self, clip, query, query_frame_bbox=None, training=False, fix_backbone=True):
        ...
        if fix_backbone:
            with torch.no_grad():
                ...
                query_feat1 = self.extract_feature(query[:,0])
                query_feat2 = self.extract_feature(query[:,1])
                query_feat3 = self.extract_feature(query[:,2])
                query_feat = torch.stack([query_feat1, query_feat2, query_feat3], dim=0).mean(dim=0)  
            ...
            query_feat2 = self.extract_feature(query[:,1])
            query_feat3 = self.extract_feature(query[:,2])
            query_feat = torch.stack([query_feat1, query_feat2, query_feat3], dim=0).mean(dim=0)      # [b c h w]
