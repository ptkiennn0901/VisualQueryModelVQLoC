...
import itertools
import argparse
from dataset.base_dataset import QueryVideoDataset, VisualQuery2DDataset
...
def get_dataset(config, split='train'):
    ...
    elif dataset_name == 'zalo':
        abs_dir = '/home/kienpt/train/samples'
        data_paths = glob.glob(abs_dir+'/*')
        data_paths = [p.replace('\\', '/') for p in data_paths]

        train_paths = data_paths[2:12]
        val_paths = data_paths[:2] + data_paths[12:]
